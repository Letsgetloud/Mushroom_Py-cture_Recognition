...
def app():
    ...
    c2.markdown(f'''The Mushroom Pyc-ture Recognition project was carried out as part of a Data Scientist training course at the DataScientest institute from March to September 2021. (<a href="https://datascientest.com">datascientest.com</a>)''', unsafe_allow_html=True)
    st.write("")
    c1, c2, c3  = st.columns([0.5, 1, 1])
    with c1:
        st.markdown(f'''<u>Project members</u> :''', unsafe_allow_html=True)  
    with c2:
        logo_linkedin = get_img_with_href(os.path.join(currentdir, '_assets_/images/linkedin.png'), 'https://www.linkedin.com/in/david-cen', 20)
        st.markdown(f'''<a href="https://www.linkedin.com/in/david-cen" style="text-decoration: none;color:black">David CHARLES-ELIE-NELSON</a> {logo_linkedin}''', unsafe_allow_html=True) 
    with c3:
