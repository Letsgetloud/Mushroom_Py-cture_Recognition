...
class CModelSelector(CBaseComponent):
   ...
   def getSelectedModel(self):
      idx = ss.get(ssKeys.INTERPRET_CHOICE_MODEL_IDX.value)
   ...
   def render(self):
      ...
      with st.container():
         ...
         if self.models is None:
            st.warning('No model available for this application. You should contact the administrator to check if this is an expected situation.')
            return

         modelNames = [ model.getName() for model in self.models ]

         #
         # Selectbox: model - selector
         #
         choiceModelIdx = st.selectbox(
               label       = 'Model Selection'
            ,  key         = ssKeys.INTERPRET_CHOICE_MODEL_IDX.value
            ,  options     = np.arange(len(modelNames))
            ,  index       = 0
            ,  format_func = lambda idx: modelNames[idx]
            ,  help        = 'Choose the model for which you want to choose its interpretability.'
         )
         ...
         classes = self.models[choiceModelIdx].getClasses()
         if classes is None or len(classes) < 1:
            st.warning('No class associated to this model')
         else:
            with st.expander('Model Description', expanded = True):
               classesListStr = ''
               for className in classes:
                  classesListStr += '   - ' + str(className.value) + '\n'
               modelDesc = f'You have choosen a model that is able to classifiy the following genuses:\n{classesListStr}'
               st.write(modelDesc)
