...
class CImageRecognition(CBaseComponent):
   ...
   def _classify(self
      ...
      with st.expander('Details for each underlying model :'):
        ...
        col3.write(classification)

        col4.markdown('* Classification per member:')
        col4.dataframe(
         df_membersClass.apply(func = lambda x: classNames[x].value)
         )
   ...
   def doModelInterpretability(self, console):
           ...
           grad_cam = self.perform_GradCAM(models=self.mushroomClassifier, image=self.image, console=console, progressBar=progressBar)
   ...
   def render(self):
      ...
      with st.container():
         ...
         btn_LaunchRecognition_2 = st.button(
            ...
            ,  help  = 'Show heatmaps of underlying CNN models'
            )

         if btn_LaunchRecognition_2:
                    console = st.empty()
                    pp2.write(self.doModelInterpretability(console = console))
